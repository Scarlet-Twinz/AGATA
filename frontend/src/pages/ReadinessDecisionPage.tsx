...
type Readiness = { status: string; score?: number | null; explanation?: string; [key: string]: unknown };
...
export default function ReadinessDecisionPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [decisionData, readinessData] = await Promise.all([
        api<Decision>(`/api/readiness/projects/${projectId}/contractors/${contractorId}/decision`),
        api<Readiness>(`/api/readiness/projects/${projectId}/contractors/${contractorId}`),
      ]);
      setDecision(decisionData); setReadiness(readinessData);
  ...
  if (!decision || !readiness) return <section className="decision-page"><div className="decision-card"><strong>Decision unavailable</strong><p>{error || "This readiness decision could not be found."}</p><button onClick={() => navigate("/readiness")}>Back to readiness</button></div></section>;
  ...
  return <><style>{styles}</style><section className="decision-page">
    ...
    <div className="decision-grid">
      <article className="decision-card decision-summary"><span className="decision-label">READINESS RESULT</span><div className={`decision-result ${ready ? "ready" : "blocked"}`}>{label(readiness.status)}</div><div className="decision-score">{score == null ? "—" : `${score}%`}<small>readiness score</small></div><p>{readiness.explanation || "The readiness engine evaluates project requirements against contractor evidence."}</p></article>
