      ...
      <div className="rumi-workspace">
        <main className="rumi-chat-panel">
          ...
          <div className="rumi-messages">
            {loadingHistory ? <div className="rumi-loading">Restoring your Rumi conversation…</div> : messages.length === 0 ? <div className="rumi-welcome"><div className="rumi-welcome-mark">R</div><h2>What do you need to know?</h2><p>Ask Rumi about the compliance state of your workspace. Rumi should explain decisions rather than invent data.</p><div className="rumi-starters">{starters.map((starter) => <button key={starter} type="button" onClick={() => void sendMessage(starter)}>{starter}<span>→</span></button>)}</div></div> : messages.map((message, index) => <div className={`rumi-message ${message.role}`} key={`${index}-${message.role}`}><div className="rumi-message-label">{message.role === "user" ? "You" : "Rumi"}</div><div className="rumi-message-body">{message.content || (sending && index === messages.length - 1 ? "Thinking…" : "")}</div></div>)}
          </div>
          <form className="rumi-composer" onSubmit={submit}><textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void sendMessage(); } }} placeholder="Ask Rumi about your workspace…" rows={2} disabled={sending || loadingHistory}/><button type="submit" disabled={!canSend}>{sending ? "Thinking…" : "Ask Rumi"}<span>↗</span></button></form>
          {error && <div className="rumi-error">{error}</div>}
        </main>
        <aside className="rumi-context"><span className="rumi-eyebrow">HOW RUMI WORKS</span><h2>Explain the decision.</h2><p>Rumi conversations are stored securely for your AGATA account and workspace, while business answers remain grounded in current workspace data.</p><div className="rumi-context-list"><div><b>Persistent conversation</b><span>Your chat returns after a refresh or later session.</span></div><div><b>Evidence</b><span>Understand validity, expiry, and mapping.</span></div><div><b>Readiness</b><span>Understand why a contractor is Ready, Attention, or Not ready.</span></div></div></aside>
      </div>
    </section>
  </>;
}
