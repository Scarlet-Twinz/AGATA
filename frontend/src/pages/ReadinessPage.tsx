...
function decisionLabel(status: Item["decision_status"]) {
  if (status === "approved") return "Approved";
  if (status === "rejected") return "Rejected";
  if (status === "pending") return "Pending decision";
  return "Decision not recorded";
}
function decisionClass(status: Item["decision_status"]) {
  if (status === "approved") return "approved";
  if (status === "rejected") return "rejected";
  ...
  return <>
    ...
    <section className="readiness-page">
      ...
      <section className="readiness-panel">
        <div className="readiness-table-head"><span>Project / Contractor</span><span>Readiness</span><span>Score</span><span>Decision</span><span>Last checked</span><span/></div>
        {loading ? <div className="readiness-state"><div className="readiness-spinner"/><span>Loading readiness…</span></div> : filtered.length === 0 ? <div className="readiness-empty"><div className="readiness-empty-icon"><Icon name="check" size={22}/></div><strong>{items.length ? "No matching readiness checks" : "No contractor assignments yet"}</strong><span>{items.length ? "Try a different search or filter." : "Assign contractors to projects first. Readiness checks will appear here."}</span></div> : <div>{filtered.map((item) => { const key = `${item.project_id}:${item.contractor_id}`; const status = statusLabel(item.status); return <article className="readiness-row" key={key}>
          ...
          <div><span className={`decision-chip ${decisionClass(item.decision_status)}`}>{decisionLabel(item.decision_status)}</span>{item.decision_reason && <p className="decision-reason-preview">{item.decision_reason}</p>}</div>
          ...
          <div className="readiness-actions"><button className="readiness-action" disabled={working === key} onClick={() => void evaluate(item)}>{working === key ? "Checking…" : item.evaluated ? "Re-check" : "Evaluate"}<Icon name="arrow" size={14}/></button>{item.evaluated && <button className="readiness-review" onClick={() => navigate(`/projects/${item.project_id}/contractors/${item.contractor_id}`)}>Review decision</button>}</div>
        </article>; })}</div>}
      </section>
