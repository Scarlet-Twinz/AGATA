...
export function AppShell() {
  ...
  const [collapsed, setCollapsed] = React.useState(() =>
    localStorage.getItem("agata_sidebar_collapsed") === "true",
  );
  ...
  const current =
    allLinks.find(([, to]) => location.pathname === to)?.[0] ??
    "Command Center";
  ...
  return (
    ...
    >
      {/* The scrim must not become a CSS-grid column. */}
      <button
      ...
      <aside
        className="sidebar"
        aria-label="AGATA workspace navigation"
        style={{
          gridColumn: 1,
          gridRow: 1,
          minWidth: 0,
        ...
        <header className="workspace-topbar">
          ...
          <div className="topbar-tools">
            ...
            <button
              ...
              aria-label={`Open settings for ${firstName}`}
            >
              {(user?.full_name ?? "A").charAt(0).toUpperCase()}
