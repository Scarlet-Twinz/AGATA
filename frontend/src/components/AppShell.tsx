    ...
    <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="sidebar">
        <div className="workspace-brand">
          ...
          <span className="workspace-brand-name">AGATA</span>
          <button className="sidebar-collapse" type="button" onClick={() => setCollapsed(v => !v)} aria-label="Toggle sidebar">{collapsed ? "→" : "←"}</button>
        </div>
            ...
            <div className="nav-group" key={group.label}>
              ...
              {group.links.map(([label, to]) => (
                <NavLink key={to} to={to} title={label} className={({ isActive }) => isActive ? "nav-link active" : "nav-link"}>
                  <span className="nav-symbol">{symbols[label] ?? label.charAt(0)}</span><span className="nav-label">{label}</span>
                </NavLink>
              ))}
            </div>
          ))}
        ...
        <div className="sidebar-bottom">
          <div className="plan-mini"><span>Current plan</span><strong>Starter</strong><NavLink to="/billing">Manage plan <b>→</b></NavLink></div>
          <div className="sidebar-user"><span className="user-avatar">{(user?.full_name ?? "A").charAt(0).toUpperCase()}</span><div className="user-meta"><strong>{user?.full_name ?? "Workspace"}</strong><span>{user?.email ?? ""}</span></div><button type="button" onClick={handleSignOut} aria-label="Sign out">↗</button></div>
        </div>
      </aside>
      <main className="main-content">
        <header className="workspace-topbar">
          <div className="topbar-left"><button className="mobile-sidebar-button" type="button" onClick={() => setCollapsed(v => !v)}>☰</button><div><span className="topbar-kicker">Workspace</span><span className="topbar-section">{current}</span></div></div>
          <div className="topbar-tools">
            <label className="workspace-search"><span>⌕</span><input aria-label="Search workspace" placeholder="Search workspace" /></label>
