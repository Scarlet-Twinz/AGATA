  ...
  return (
    <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom"><div className="plan-mini"><span>Current plan</span><strong>Starter</strong><NavLink to="/billing">Manage plan <b>→</b></NavLink></div><div className="sidebar-user"><span className="user-avatar">{(user?.full_name ?? "A").charAt(0).toUpperCase()}</span><div className="user-meta"><strong>{user?.full_name ?? "Workspace"}</strong><span>{user?.email ?? ""}</span></div><button type="button" onClick={handleSignOut} aria-label="Sign out">↗</button></div></div>
      </aside>
