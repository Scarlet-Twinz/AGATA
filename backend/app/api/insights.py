...
from app.models.entities import ComplianceCheck, Contractor, Document, DocumentRequirementMatch, Project, ProjectRequirement, Requirement, User
...
def insights(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    for check in sorted(checks, key=lambda x: x.checked_at):
        latest[(check.project_id, check.contractor_id)] = check
    ...
    recent_checks = [check for check in checks if check.checked_at >= cutoff]
    ...
    return {
        ...
        "signals": [
            ...
            {"key": "evaluation_coverage", "label": "Evaluation coverage", "value": round(len(latest) / max(projects * contractors, 1) * 100), "description": "Project-contractor pairs with a recorded readiness decision."},
        ],
    }
