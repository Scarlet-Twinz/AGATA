...
def list_invitations(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.execute(select(WorkspaceInvitation).where(WorkspaceInvitation.company_id == user.company_id).order_by(WorkspaceInvitation.created_at.desc())).scalars().all()
...
def create_invitation(payload: InvitationCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    item = WorkspaceInvitation(company_id=user.company_id, email=str(payload.email).lower(), role=payload.role, token=str(uuid4()), status="pending")
...
def revoke_invitation(invitation_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if not item: raise HTTPException(status_code=404, detail="Invitation not found")
    item.status = "revoked"; item.revoked_at = datetime.now(timezone.utc); db.commit()
