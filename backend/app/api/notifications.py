...
def _sync_notifications(db: Session, user: User) -> None:
    ...
    contractors = {
        ...
        for contractor in db.scalars(
            select(Contractor).where(
                Contractor.company_id == user.company_id,
                Contractor.id.in_(contractor_ids) if contractor_ids else False,
            )
        ).all()
    }
    mapped_ids = {
        ...
        for match in db.scalars(
            select(DocumentRequirementMatch).where(
                DocumentRequirementMatch.document_id.in_([document.id for document in documents])
                if documents
                else False
            )
        ).all()
    ...
    for document in documents:
        ...
        if days is not None and days < 0:
            current[f"evidence:{document.id}:expiry"] = {
                "kind": "evidence_expired",
                "severity": "critical",
                "title": f"Evidence expired: {document.name}",
                "description": f"{contractor_name}'s {document.document_type} evidence expired. Update or replace it before relying on this evidence for readiness.",
                "href": "/evidence",
            }
            ...
            current[f"evidence:{document.id}:expiry"] = {
                ...
                "severity": "warning",
                "title": f"Evidence expiring soon: {document.name}",
                "description": f"{contractor_name}'s {document.document_type} evidence expires in {max(days, 0)} day(s). Review it before it becomes invalid.",
                "href": "/evidence",
            }
        if document.id not in mapped_ids:
            current[f"evidence:{document.id}:mapping"] = {
                "kind": "evidence_unmapped",
                "severity": "info",
                "title": f"Evidence needs mapping: {document.name}",
                "description": f"{document.name} is active but is not mapped to a requirement, so it cannot contribute to a readiness decision.",
                "href": "/evidence",
            }
    ...
    check_rows = db.scalars(
        select(ComplianceCheck).where(ComplianceCheck.company_id == user.company_id).order_by(ComplianceCheck.checked_at.desc())
    ).all()
    ...
    project_map = {
        ...
        for project in db.scalars(select(Project).where(Project.company_id == user.company_id)).all()
    }
    contractor_map = {
        contractor.id: contractor.name
        for contractor in db.scalars(select(Contractor).where(Contractor.company_id == user.company_id)).all()
    }
    ...
    for assignment in assignments:
        ...
        if check is None:
            current[source_key] = {
                "kind": "readiness_unevaluated",
                "severity": "info",
                "title": f"Readiness check needed: {contractor_name}",
                "description": f"{contractor_name} is assigned to {project_name}, but readiness has not been evaluated yet.",
                "href": "/readiness",
            }
            continue
        ...
        if result["status"] == "not_ready":
            ...
            current[source_key] = {
                ...
                "severity": "critical",
                "title": f"Contractor not ready: {contractor_name}",
                "description": f"{contractor_name} is not ready for {project_name}. Missing or invalid evidence: {detail}.",
                "href": "/readiness",
            }
        elif result["status"] == "attention":
            current[source_key] = {
                "kind": "readiness_attention",
                "severity": "warning",
                "title": f"Readiness needs attention: {contractor_name}",
                "description": f"{contractor_name} needs attention before being considered fully ready for {project_name}.",
                "href": "/readiness",
            }
    ...
    for source_key, payload in current.items():
        ...
        if item is None:
            ...
            item.status = "active"
            if item.status == "active" and item.read_at is not None and item.kind != payload["kind"]:
                item.read_at = None
